 ...
 * pragmatic simplification used for Correlation Engine's Event.producedBy
 * in scenario.ts. Not something RI-001 needs to resolve.
 */
const TRUST_ENGINE_ID = "trust-engine-ri001-reference" as AgentId;
...
/**
 * Minimal, deterministic in-memory Trust Engine fixture for RI-001
 * (docs/adr/0004, docs/adr/0008). Ingests Signal/Evidence/Prediction/Outcome
 * autonomously via bus subscriptions set up in the constructor — agents
 * never call submitEvidence/submitOutcome directly, matching the ADR-0004
 * ingestion model (those methods exist to satisfy TrustEngineWriter and are
 * invoked only by this class's own subscriptions).
 *
 * The scoring rule (fixed deltas, clamped to [0, 1], starting from a 0.5
 * baseline) is a deliberately naive reference rule for RI-001, not a
 * production trust algorithm — evidence nudges the originating Sensor's
 * domain trust, resolved Outcomes nudge the predicting Agent's domain
 * trust, exactly the split described in docs/adr/0004/0008.
 */
export class InMemoryTrustEngine implements TrustEngine {
  ...
  async subscribeToTrustChanges(subject: TrustSubjectRef, domain: Domain, handler: TrustChangeHandler): Promise<Subscription> {
    ...
    return {
      id: `ri001-trust-sub-${subjectKey(subject)}@${domain}`,
  ...
  private async applyDelta(
    ...
    const next: Trust = {
      id: `trust-ri001-${this.trustSequence}` as TrustId,
      subject,
      domain,
      score: clamp01(previousScore + delta) as Confidence,
      basis,
      computedAt: at,
      computedByEngineVersion: "ri-001-reference/0.1.0",
