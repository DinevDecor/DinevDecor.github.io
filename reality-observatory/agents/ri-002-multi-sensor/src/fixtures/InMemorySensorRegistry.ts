...
/**
 * Minimal, deterministic in-memory Sensor Registry fixture for RI-002
 * (docs/adr/0005). Not a production implementation. As ADR-0005 requires,
 * every write publishes the resulting Sensor fact onto `sensor.<kind>` in
 * addition to updating internal state.
 */
export class InMemorySensorRegistry implements SensorRegistry {
  ...
  async subscribeToSensorChanges(query: SensorQuery, handler: SensorChangeHandler): Promise<Subscription> {
    ...
    const id = `ri002-sensor-sub-${this.subscriptionSequence}`;
