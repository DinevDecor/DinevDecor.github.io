 ...
 * in scenario.ts. Not something RI-002 needs to resolve.
 */
const TRUST_ENGINE_ID = "trust-engine-ri002-reference" as AgentId;
...
/**
 * Minimal, deterministic in-memory Trust Engine fixture for RI-002
 * (docs/adr/0004, docs/adr/0008). Ingests Signal/Evidence/Prediction/Outcome
 * autonomously via bus subscriptions — agents never call submitEvidence/
 * submitOutcome directly. Evidence nudges the originating Sensor's domain
 * trust (independently, per sensor — this is what lets RI-002 prove three
 * different sensors each accrue their own Domain Trust record); resolved
 * Outcomes nudge the predicting Agent's domain trust. The scoring rule
 * (fixed deltas, clamped to [0, 1]) is a deliberately naive reference rule,
 * not a production trust algorithm.
 */
export class InMemoryTrustEngine implements TrustEngine {
  ...
  async subscribeToTrustChanges(subject: TrustSubjectRef, domain: Domain, handler: TrustChangeHandler): Promise<Subscription> {
    ...
    return {
      id: `ri002-trust-sub-${subjectKey(subject)}@${domain}`,
  ...
  private async applyDelta(
    ...
    const next: Trust = {
      id: `trust-ri002-${this.trustSequence}` as TrustId,
      subject,
      domain,
      score: clamp01(previousScore + delta) as Confidence,
      basis,
      computedAt: at,
      computedByEngineVersion: "ri-002-reference/0.1.0",
