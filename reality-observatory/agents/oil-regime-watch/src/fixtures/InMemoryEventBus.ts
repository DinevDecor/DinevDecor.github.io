...
/**
 * Minimal, deterministic in-memory Event Bus fixture used to test Oil
 * Regime Watch. Delivers to matching subscribers synchronously and in
 * subscription order — not a production Event Bus (see docs/adr/0004); no
 * production runtime implementing EventBus exists yet in this repo (see
 * README, "Production readiness"), so this fixture is what lets the
 * agent's real logic run and be tested at all.
 */
export class InMemoryEventBus implements EventBus {
  ...
  async subscribe<TPayload>(
    ...
    const id = `oil-regime-sub-${this.subscriptionSequence}`;
