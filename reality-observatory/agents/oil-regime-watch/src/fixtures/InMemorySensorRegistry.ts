...
/**
 * Minimal, deterministic in-memory Sensor Registry fixture (docs/adr/0005).
 * Not a production implementation. Every write publishes the resulting
 * Sensor fact onto `sensor.<kind>` in addition to updating internal state.
 */
export class InMemorySensorRegistry implements SensorRegistry {
  ...
  async subscribeToSensorChanges(query: SensorQuery, handler: SensorChangeHandler): Promise<Subscription> {
    ...
    const id = `oil-regime-sensor-sub-${this.subscriptionSequence}`;
 ...
 * agent — the concrete `AgentContext.ownSensors` binding (docs/adr/0005).
 * Enforces `ownerAgentId === agentId` before delegating; runtime, not
 * type-level, per the carve-out in docs/adr/0011.
 */
